...
func _ready() -> void:
	...
	add_child(replay)
	replay.setup({"board": board, "play": play, "log": combat_log, "menu": menu,
		"ua": ua, "ub": ub, "hud_a": hud_a, "hud_b": hud_b, "local_is_a": _local_is_a()})
...
func _game_loop() -> void:
	...
	while true:
		...
		if opponent is AIOpponent:
			AI.forward_observation(local_plan, obs_sit)   # feed the C# brain's model (pointless + risky vs a human)
